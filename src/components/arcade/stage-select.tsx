...
export default function StageSelect({ onPick }: { onPick: (index: number) => void }) {
	...
	return (
		<div className="fixed inset-0 z-40 flex flex-col bg-[#0A0E14] p-6 sm:p-10">
			<div className="mb-6 flex items-center gap-4">
				<span className="font-mono text-[11px] tracking-[0.3em] text-[#00E5C7]">SELECT</span>
			...
			<div className="grid flex-1 gap-8 lg:grid-cols-[1.4fr_1fr]">
					...
					{fighters.map((fighter) => (
						<button
							...
							className={`group relative aspect-[3/4] overflow-hidden border bg-white/[0.02] text-left transition ${
								active === fighter.index
									? "border-[#00E5C7] shadow-[0_0_30px_-8px_#00E5C7]"
						...
						>
							...
							<div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent p-2">
								<span className="font-display text-sm font-semibold text-[#E4E7EB]">{fighter.name}</span>
							</div>
						</button>
					))}
				</div>
				...
				<div className="flex flex-col justify-end border border-white/10 bg-white/[0.02] p-6">
					<div className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#00E5C7]">{f.stage}</div>
					<h3 className="mt-2 font-display text-4xl font-bold tracking-tight text-[#E4E7EB]">{f.name}</h3>
						...
						{f.stats.map((s) => (
							<div key={s.label}>
								...
								<div className="mt-1 h-1.5 w-full overflow-hidden bg-white/10">
									<div className="h-full bg-[#00E5C7]" style={{ width: `${s.value}%` }} />
								</div>
							</div>
						))}
					</div>
					<button
						...
						className="mt-8 w-full -skew-x-6 bg-[#00E5C7] py-3 font-mono text-xs font-bold uppercase tracking-[0.3em] text-[#0A0E14] transition hover:brightness-110"
					>
						Fight →
					</button>
				</div>
